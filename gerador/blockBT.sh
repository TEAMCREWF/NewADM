...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://www.dropbox.com/s/q1oq2c6ex3n060c/blockBT.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} No se puede vincular a Github !" && exit 0
	wget https://www.dropbox.com/s/q1oq2c6ex3n060c/blockBT.sh -O /etc/ger-frm/blockBT.sh &> /dev/null
